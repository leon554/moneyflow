...
export class IncomeSource implements ISimulatable{
    ...
    public step(date: Date, mutate: boolean, buckets: Map<string, Bucket>): IPayment[]{
        ...
        mutate && (this.sourceData.nextIncurralDate = Util.getNextDate(new Date(nextIncurralDate), incomeFrequency).getTime())
        ...
        for(const bucketId of this.destinationBucketsIds){
            ...
            mutate && (this.totalPaid += paymentAmount)
            mutate && (bucket.addMoney(paymentAmount))
    ...
    public getAllocatedData(allocations: Source[], excludeSourceIds: string[], buckets: Map<string, Bucket>){
        ...
        excludeSourceIds.forEach(id =>  allAllocations.delete(id))
    ...
    public getAllocatedDataWithTemp(allocations: Source[], tempSource: Source, excludeSourceIds: string[], buckets: Map<string, Bucket>){
        const data = this.getAllocatedData(allocations, excludeSourceIds, buckets)
    ...
    public canAffordAllocation(allocations: Source[], excludeSourceIds: string[], buckets: Map<string, Bucket>): boolean{
        const {unAllocatedAmount} = this.getAllocatedData(allocations, excludeSourceIds, buckets)
